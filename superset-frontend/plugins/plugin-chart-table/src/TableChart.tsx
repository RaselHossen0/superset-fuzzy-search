...
import { isEmpty, debounce, isEqual, escapeRegExp } from 'lodash';
...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  const handleFilteredDataChange = useCallback(
    (rows: Row<D>[], filterText?: string) => {
      const trimmedFilterText = filterText?.trim() || '';
      setSearchText(trimmedFilterText);
  ...
  useEffect(() => {
    if (serverPagination) {
      setSearchText(serverPaginationData?.searchText?.trim() || '');
    }
  }, [serverPagination, serverPaginationData?.searchText]);
