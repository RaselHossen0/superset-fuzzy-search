...
export default typedMemo(function DataTable<D extends object>({
  ...
  const defaultGlobalFilter: FilterType<D> = useCallback(
    (rows: Row<D>[], columnIds: IdType<D>[], filterValue: string) => {
      const query = (filterValue ?? '').toString().trim();
      if (!query) {
        return rows;
      }

      const fuseOptions: IFuseOptions<Row<D>> = {
        // Search across each visible column's rendered value
        keys: columnIds.map(columnId => ({
          name: `values.${String(columnId)}`,
          weight: 1,
        })),
        // More lenient threshold for better typo tolerance (0.6 = more fuzzy)
        threshold: 0.6,
        ignoreLocation: true,
        minMatchCharLength: 2,
        // Include score for potential highlighting
        includeScore: true,
